        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                card("network", L("数据来源 · 用量页（配额 %）", "Data Sources · Usage Page (quota %)")) {
                    ...
                    rowDivider
                    providerRow("Claude", "~/.tokenitor/claude-statusline.json",
                                [(L("本地优先", "Local first"), .ok), (L("默认关", "Off by default"), .mut)],
                                L("**首选本地读取**：Claude Code 每轮把 `rate_limits`（5 小时 / 7 天窗口）交给 statusline 脚本，脚本落盘、本应用只读——零联网、零限流、不弹钥匙串。在「设置 → 快捷操作」一键启用（需重启 Claude Code）。**仅终端版 Claude Code 有效**——状态栏是终端 TUI 的组件，Claude 桌面 App 不渲染它，桌面 App 的会话不会产生这份数据。未启用时回落到社区接口 `api/oauth/usage`：凭证只读、**不代它续期**、诚实 UA，且该端点被官方判为 not planned 且限流极猛，故最短 600s 才调一次。再失败则显示 24 小时内的缓存（`缓存` 胶囊），超期如实报错。", "**Local first**: each turn, Claude Code hands `rate_limits` (5-hour / 7-day windows) to a statusline script, which writes them to a file this app reads — no network, no rate limits, no Keychain prompt. Enable it in Settings → Quick Actions (restart Claude Code afterwards). **Terminal Claude Code only** — the status line is a terminal-TUI component, so the Claude desktop app never renders it and its sessions produce no such data. Without it, falls back to the community `api/oauth/usage` endpoint: credentials read-only, **never refreshed by us**, honest UA — and since Anthropic marked that endpoint not planned and it rate-limits hard, it is called at most once per 600s. Failing that, cached data under 24h is shown with a `Cached` chip; older than that it reports an honest error."))
