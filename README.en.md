...
| Tool | Source | Notes |
|------|--------|-------|
| Claude | `~/.tokenitor/claude-statusline.json` (preferred) · `api.anthropic.com/api/oauth/usage` (fallback) | **Local first**: each turn Claude Code hands `rate_limits` (5-hour / 7-day windows with `used_percentage` / `resets_at`) to a statusline script; the bundled script writes it to a file this app reads — **no network, no rate limits, no Keychain prompt**, and no ToS exposure. Enable it via Settings → Quick Actions → Enable Local Read (backs up your `~/.claude/settings.json`; restart Claude Code afterwards). **⚠️ Terminal Claude Code only**: the status line is a terminal-TUI component, so the Claude desktop app never renders it and its sessions produce no such data — those fall back to the community endpoint below. Without it, falls back to the community endpoint: the token is read from `~/.claude/.credentials.json` or the Keychain (**never refreshed by us**) with an honest UA. Anthropic marked that endpoint `not planned` and it rate-limits hard (community consensus polls it every 300–900s), so it is called at most once per **600s**. Failing that, cached data under 24h is shown with a `Cached` chip; older than that it reports an honest error. **⚠️ Advanced — off by default.** |
